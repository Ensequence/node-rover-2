...
module.exports = function (callback) {
    // First, get rover params
    request.get('http://' + ROVER_HOST + ':' + ROVER_PORT + '/get_params.cgi', {
        auth: {
            user: config.ROVER_USERNAME,
            pass: config.ROVER_PASSWORD
        }
    }, function (err, res, body) {
        // Capture rover params
        body.replace(/var ([a-zA-Z_]+)=\'?([a-zA-Z0-9\._]+)\'?;/g, function (match, p1, p2) {
            config[p1] = p2;
        });
...
function init (callback) {
    // build key
    config.key = [config.ROVER_USERNAME, config.id + '-save-private', config.ROVER_PASSWORD].join(':');

    // Initialize blowfish
    blowfish.init(config.key);
